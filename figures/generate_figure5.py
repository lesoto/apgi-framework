"""Figure 5 — Protocol 3 — Anticipation-fMRI: vmPFC–insula anticipatory coupling (Pred 3.a–Pred 3.d).

Simulates fMRI PPI coefficients and SCR outcomes from protocol_3_anticipation_fmri.json. Shows:
  A — vmPFC–pIC PPI: anticipation > outcome (Pred 3.b)
  B — vmPFC BOLD parametric modulation: valence vs. contrast sensitivity (Pred 3.c)
  C — PPI coefficient: long foreperiod vs. no-foreperiod control (Pred 3.d)
...
def plot(data: dict, show: bool = True) -> None:
    ...
    label_axes(axes)
    fig.suptitle(
        "Figure 5 — Protocol 3 — Anticipation-fMRI: vmPFC–Insula Anticipatory Coupling",
        fontsize=11,
        y=1.02,
    )
    fig.tight_layout()
