...
_EX = "#2166ac"   # excitatory
_IN = "#d6604d"   # inhibitory
_DIS = "#4dac26"  # disinhibition


def _draw_node(ax, xy, label, color="#f7f7f7", edgecolor="#333333", r=0.06, fontsize=8):
    circ = mpatches.Circle(xy, r, facecolor=color, edgecolor=edgecolor, lw=1.5, zorder=3)
    ax.add_patch(circ)
    ax.text(xy[0], xy[1], label, ha="center", va="center", fontsize=fontsize,
            fontweight="bold", zorder=4, wrap=True, multialignment="center")
...
def _arrow(ax, src, dst, color, style="->", lw=1.5, label=None, label_offset=(0, 0.04)):
    ...
    if label:
        mid = ((src[0] + dst[0]) / 2 + label_offset[0],
...
def draw_main_circuit(ax):
    ...
    _arrow(ax, (nodes["vmPFC"][0] + 0.075, nodes["vmPFC"][1]),
               (nodes["aInsula"][0] - 0.075, nodes["aInsula"][1]),
               _EX, label="direct\nprojection", label_offset=(0, 0.06))

    # aInsula → VIP (excitatory)
    _arrow(ax, (nodes["aInsula"][0] + 0.075, nodes["aInsula"][1]),
               (nodes["VIP"][0] - 0.075, nodes["VIP"][1]),
               _EX)

    # VIP → SST (inhibitory)
    _arrow(ax, (nodes["VIP"][0], nodes["VIP"][1] - 0.075),
               (nodes["SST"][0], nodes["SST"][1] + 0.075),
               _IN, label="inhibition", label_offset=(0.06, 0))

    # SST → L23pyr (releases inhibition → disinhibition arrow)
    _arrow(ax, (nodes["SST"][0] + 0.075, nodes["SST"][1]),
    ...
    # Result annotation
    ax.text(
        nodes["L23pyr"][0], nodes["L23pyr"][1] - 0.14,
        r"$\Pi^i_{\mathrm{eff}}$ elevation",
        ha="center", va="top", fontsize=8, color=_DIS, fontweight="bold",
    )

    # Legend
    for color, label in [(_EX, "Excitatory"), (_IN, "Inhibitory"), (_DIS, "Disinhibition")]:
        ax.plot([], [], color=color, lw=2, label=label)
    ax.legend(loc="lower left", fontsize=7, framealpha=0.8)

    ax.set_title("Proposed circuit — vmPFC→insula→VIP+→SST+→L2/3 pyramid\n(convergent anatomical evidence; causal validation pending)", fontsize=8.5)
...
def draw_state_inset(ax, title, beta_sm_state):
    ...
    dend_color = "#4dac26" if active else "#aaaaaa"
    ...
    for x, y, lbl, col, alpha in items:
        circ = mpatches.Circle(
            (x, y), 0.10, facecolor=col, edgecolor="#333333",
            lw=1.2, alpha=alpha, zorder=3,
        )
        ax.add_patch(circ)
        ax.text(x, y, lbl, ha="center", va="center", fontsize=6.5, zorder=4,
                alpha=min(alpha + 0.2, 1.0), fontweight="bold", multialignment="center")
    ...
    pi_label = r"high $\Pi^i_{\mathrm{eff}}$" if active else r"low $\Pi^i_{\mathrm{eff}}$"
    ax.text(0.75, 0.35, pi_label, ha="center", fontsize=7, color=dend_color, fontweight="bold")

    bsm_label = r"$\beta_{\mathrm{SM}}$ high" if active else r"$\beta_{\mathrm{SM}}$ low"
