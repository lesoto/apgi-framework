...
EPOCHS = [
    {
        ...
        r"$\Pi^i_{\mathrm{eff}}$ modulation measurement window",
        "t": 0.25,
        "dur": 0.18,
        "color": "#9ecae1",
    },
...
def plot(show: bool = True) -> None:
    ...
    for cond in CONDITIONS:
        y = cond["y"]
        # Per-condition shading band so the three tracks are visually
        # distinguishable (tint shows above/below the shared event boxes).
    ...
    # Clarify that the three conditions share timing/events (reproducibility)
    ax.text(
        0.5,
        3.92,
