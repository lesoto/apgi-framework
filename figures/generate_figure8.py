"""Figure 8 — Protocol 6: iEEG all-or-none ignition dynamics (Pred 6.A–Pred 6.D).
...
def plot(data: dict, show: bool = True) -> None:
    ...
    ax.axvspan(200, 400, alpha=0.10, color=PALETTE["S_t"], label="Peak window 200–400 ms")
    ax.axhline(0.4, ls="--", lw=0.9, color=PALETTE["S_t"], alpha=0.5, label="r > 0.4 threshold")
    ax.axhline(0.20, ls="--", lw=0.9, color="#AAAAAA", alpha=0.5, label="r < 0.20 (occipital)")
    ...
    label_axes(axes)
    fig.suptitle(
        "Figure 8 — Protocol 6: iEEG All-or-None Ignition Dynamics (Pred 6.A–Pred 6.D)", fontsize=11, y=1.02
    )
    fig.tight_layout()
