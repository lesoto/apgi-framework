...
def plot(show: bool = True) -> None:
    ...
    ax1.fill_between(rho, 0, signal_low, alpha=0.25, color="#6baed6",
                     label="Sub-threshold basin")
    ax1.fill_between(rho, np.where(rho >= rho_crit, signal_low, np.nan),
                     np.where(rho >= rho_crit, signal_high, np.nan),
                     alpha=0.25, color="#d6604d", label="Ignition zone")
    ax1.plot(rho, signal_low, lw=1.8, color="#2166ac")
    ax1.plot(rho[rho >= rho_crit], signal_high[rho >= rho_crit], lw=1.8, color="#d6604d")

    # Critical manifold boundary
    ax1.axvline(rho_crit, lw=1.8, ls="--", color="#333333")
    ax1.annotate(
        r"Saddle-node: $\rho_{\mathrm{crit}} \approx 0.95$",
        xy=(rho_crit, 0.5), xytext=(rho_crit + 0.08, 0.55),
        fontsize=8,
    ...
    ax1.text(rho_crit / 2, 0.02, "Sub-threshold\nbasin", ha="center", fontsize=8, color="#2166ac")
    ax1.text(1.2, 1.1, "Ignition\nzone", ha="center", fontsize=8, color="#d6604d")
    ax1.text(rho_crit + 0.01, 0.02, "Critical\nmanifold", ha="left", fontsize=7.5,
    ...
    ax2.annotate(
        r"$\alpha_{psy} \geq 10$" + "\n(predicted range)",
        xy=(1.6, 0.75), fontsize=8, color="#d6604d",
        ha="center",
    )
    ax2.set_xlabel(r"$|y(t)| / \theta_t$", fontsize=10)
    ax2.set_ylabel(r"$P(\mathrm{ignition})$", fontsize=10)
    ax2.set_title("Ignition-probability sigmoid family\n(steepness ∝ Π(t))", fontsize=10, fontweight="bold")
    ...
    fig.text(0.5, -0.03, caption_note, ha="center", fontsize=7.5, color="#666666",
             style="italic")
