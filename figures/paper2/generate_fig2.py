...
LAYER_COLORS = {
    "L1": "#e5f5e0", "L2/3": "#c7e9c0", "L4": "#a1d99b",
    "L5": "#74c476", "L6": "#41ab5d",
}
LAYER_H = 0.13
...
def draw_laminar_column(ax):
    ...
    ax.set_ylim(0.1, 1.0)
    ax.axis("off")
    ax.set_title("A — Laminar column\n(receptor assignments)", fontsize=9, fontweight="bold")

    for name, y in LAYER_Y.items():
        rect = mpatches.FancyBboxPatch(
            (0.15, y), 0.5, LAYER_H,
            boxstyle="square,pad=0.01",
            facecolor=LAYER_COLORS[name], edgecolor="#555555", lw=1.0, zorder=3,
        )
        ax.add_patch(rect)
    ...
    for layer, text, x, color in annotations:
        y = LAYER_Y[layer]
        ax.text(x, y + LAYER_H / 2, text, ha="left", va="center", fontsize=6.5,
                color=color, style="italic")
        ax.plot([0.65, x - 0.02], [y + LAYER_H / 2, y + LAYER_H / 2],
                lw=0.8, color=color, alpha=0.5)

    ax.text(0.40, 0.16, "Proposed circuit — causal dissociation requires\n"
...
def draw_pathway_panel(ax):
    ...
    ax.set_ylim(0, 1)
    ax.axis("off")
    ax.set_title("B — vmPFC→VIP+→SST+ disinhibition\n(baseline vs. active)", fontsize=9, fontweight="bold")
    ...
    for group, label, offset_x in [(nodes_base, "Baseline\n(β_SM low)", 0.05),
                                    (nodes_act, "Active\n(β_SM high)", 0.55)]:
        ax.text(offset_x + 0.15, 0.96, label, ha="center", fontsize=8,
                fontweight="bold", color="#333333")
        for x, y, name, color, alpha in group:
            circ = mpatches.Circle((x, y), 0.07, facecolor=color, edgecolor="#333333",
                                   lw=1.2, alpha=alpha, zorder=3)
            ax.add_patch(circ)
    ...
    arrow_kw = dict(xycoords="data", textcoords="data",
                    arrowprops=dict(arrowstyle="->", lw=1.5), zorder=5)
    ax.annotate("", xy=(0.60, 0.75), xytext=(0.60, 0.67),
                arrowprops=dict(arrowstyle="->", color=_EX, lw=1.5))
    ax.annotate("", xy=(0.60, 0.45), xytext=(0.60, 0.53),
                arrowprops=dict(arrowstyle="->", color=_IN, lw=1.5,
                                linestyle="dashed"))
    ax.annotate("", xy=(0.83, 0.50), xytext=(0.67, 0.50),
...
def draw_pyramidal_panel(ax):
    ...
    ax.set_ylim(0, 1)
    ax.axis("off")
    ax.set_title("C — L5 pyramidal: apical/basal\nmismatch → burst signal", fontsize=9, fontweight="bold")

    # Cell body
    soma = mpatches.Circle((0.50, 0.35), 0.07, facecolor="#74c476",
                            edgecolor="#333333", lw=1.5, zorder=3)
    ax.add_patch(soma)
    ax.text(0.50, 0.35, "L5\nsoma", ha="center", va="center", fontsize=7, zorder=4)

    # Apical dendrite
    ax.annotate("", xy=(0.50, 0.82), xytext=(0.50, 0.42),
                arrowprops=dict(arrowstyle="-", color="#555555", lw=2.5))
    rect_apic = mpatches.FancyBboxPatch(
        (0.35, 0.82), 0.30, 0.10,
        boxstyle="round,pad=0.01", facecolor="#cce5ff", edgecolor="#2166ac", lw=1.5,
    )
    ax.add_patch(rect_apic)
    ax.text(0.50, 0.87, "Apical dendrite\n(top-down prediction)", ha="center",
            va="center", fontsize=6.5, color="#2166ac")

    # Basal dendrite
    ax.annotate("", xy=(0.50, 0.12), xytext=(0.50, 0.28),
                arrowprops=dict(arrowstyle="-", color="#555555", lw=2.5))
    rect_bas = mpatches.FancyBboxPatch(
        (0.35, 0.05), 0.30, 0.08,
        boxstyle="round,pad=0.01", facecolor="#fdd49e", edgecolor="#d6604d", lw=1.5,
    )
    ax.add_patch(rect_bas)
    ax.text(0.50, 0.09, "Basal (thalamic\nbottom-up)", ha="center",
            va="center", fontsize=6.5, color="#d6604d")

    # NMDA plateau annotation
    ax.annotate(
        "NMDA plateau →\nburst (εᵉ/εⁱ)",
        xy=(0.57, 0.60), xytext=(0.78, 0.65),
        fontsize=7, color="#7b3294",
        arrowprops=dict(arrowstyle="->", color="#7b3294", lw=1.0),
    )

    ax.text(0.50, 0.00, "Thalamic/claustral substrates not\ntranscranially accessible (§5.3)",
