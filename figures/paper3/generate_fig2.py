...
def plot(show: bool = True) -> None:
    ...
    for name, tau in LEVEL_TAUS.items():
        ...
        if name in ("L0", "L1", "L2"):
            S_partial += S_lev
        ax.loglog(f, S_lev, lw=1.2, color=LEVEL_COLORS[name], linestyle="--",
                  alpha=0.7, label=f"{name} (τ≈{tau:.3g} s)")
        # Corner freq label
        fc = 1 / (2 * np.pi * tau)
        ax.axvline(fc, lw=0.7, color=LEVEL_COLORS[name], alpha=0.35, ls=":")

    # Composite (full hierarchy)
    ax.loglog(f, S_total, lw=2.5, color="#333333", label=r"$S_{\mathrm{total}}(f)$ — full (H ≈ 0.85–0.95)")

    # Partial hierarchy (L0–L2 only)
    ax.loglog(f, S_partial, lw=2.0, color="#999999", linestyle="-.",
              label="L0–L2 only — EEG amplitude-envelope (H ≈ 0.65–0.75)")

    # White noise baseline
    ax.axhline(1e-3, lw=1.0, ls=":", color="#aaaaaa", alpha=0.7)
    ax.text(1e-6, 1.2e-3, "White noise baseline (H = 0.5)", fontsize=7.5, color="#aaaaaa")
    ...
    for fc, label in crossovers:
        ...
        ax.text(fc * 1.1, S_total[np.argmin(np.abs(f - fc))] * 1.5, label,
                fontsize=7, color="#555555", va="bottom")

    # Mid-band slope annotation
    ax.text(1e-3, 50, r"$H \approx 0.85{-}0.95$" + "\n(full hierarchy)", fontsize=9,
            color="#333333", fontweight="bold")
    ax.text(3e-3, 3, r"$H \approx 0.65{-}0.75$" + "\n(L0–L2)", fontsize=8.5,
            color="#666666")
