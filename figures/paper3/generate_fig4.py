...
def draw_disorder_column(ax, disorder):
    ...
    ax.axis("off")
    ax.set_title(disorder["name"], fontsize=10, fontweight="bold",
    ...
    for i, (lv_name, base_color, lv_spec) in enumerate(
        ...
        # Level name
        ax.text(BOX_X - 0.02, y + BOX_H / 2, lv_name,
        ...
        if theta_dir != 0:
            ...
            ax.text(BOX_X + 0.12, y + BOX_H * 0.7, f"θ{arrow}",
                    ha="center", fontsize=7.5, color="#d6604d", fontweight="bold")

        # Π annotation
        pi_dir = lv_spec["pi"]
        if pi_dir != 0:
            arrow = "↑" * abs(pi_dir) if pi_dir > 0 else "↓" * abs(pi_dir)
            ax.text(BOX_X + 0.45, y + BOX_H * 0.3, f"Π{arrow}",
                    ha="center", fontsize=7.5, color="#2166ac", fontweight="bold")
    ...
    if cf is not None and ct is not None:
        ...
        ax.text(BOX_X + BOX_W + 0.10, (y_from + y_to) / 2,
                "cascade\n(α disrupted)", fontsize=6.5, color="#e6a817",
                ha="left", va="center")

    # Caption
    ax.text(0.50, -0.07, disorder["caption"], ha="center", va="top",
            fontsize=7, color=disorder["color"], style="italic", multialignment="center")
...
def plot(show: bool = True) -> None:
    ...
    handles = [
        ...
        mpatches.Patch(facecolor="none", edgecolor="#333333", lw=3, label="Primary site"),
    ]
    fig.legend(handles=handles, loc="lower center", fontsize=8, ncol=4,
