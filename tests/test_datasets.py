...
class TestMakeSampleSession:
    ...
    def test_values_are_in_expected_ranges(self):
        ...
        assert np.all(result["pi_e"] >= 0.8) and np.all(
            result["pi_e"] <= 1.5
        )
        assert np.all(result["z_e"] >= 0.2) and np.all(
            result["z_e"] <= 1.0
        )
        assert np.all(result["pi_i"] >= 0.5) and np.all(
            result["pi_i"] <= 1.5
        )
        assert np.all(result["z_i"] >= 0.1) and np.all(
            result["z_i"] <= 0.8
        )
...
class TestMakeSampleDocGroups:
    ...
    def test_reproducibility_with_seed(self):
        result1 = datasets.make_sample_doc_groups(
            n_per_group=2, n_trials=5, seed=42
        )
        result2 = datasets.make_sample_doc_groups(
            n_per_group=2, n_trials=5, seed=42
        )
        for key in result1:
    ...
    def test_different_seeds_produce_different_results(self):
        result1 = datasets.make_sample_doc_groups(
            n_per_group=2, n_trials=5, seed=1
        )
        result2 = datasets.make_sample_doc_groups(
            n_per_group=2, n_trials=5, seed=2
        )
        # At least one key should differ with different seeds
        any_different = any(
            not np.array_equal(result1[key], result2[key])
            for key in result1
        )
